...
void setupsignals()
{
  ...
  for(int i = 0; i < LENGTH(blocks); i++)
  {
    ...
    { 
      // printf("setting up signal for %s:%d\n", blocks[i].command, SIGRTMIN+blocks[i].signal);
...
void setroot()
{
  ...
  strcpy(statusstr[2], statusstr[0]);
  for(int i = 0; i < LENGTH(blocks); i++)
    remove_all(statusstr[2], blocks[i].signal);
  XStoreName(dpy, root, statusstr[2]);
  XCloseDisplay(dpy);
}
...
void buttonhandler(int sig, siginfo_t *si, void *ucontext)
{
  ...
  // printf("receieved signal with value %d\n", sig);
